...
class SleepMeasurementsDB {
    ...
    /* Adds the SleepMeasurement object that must be passed as the first argument.
    ...
    /* Returns an array of all SleepMeasurement objects found in the database for all users.
    ...
    public static function getAllMeasurements() {
        ...
        try {
            $db = Database::getDB();
            $stmt = $db->prepare(
                "select userName, dateAndTime, duration, units, notes
    ...
    // Returns a SleepMeasurement object for the specified user with the specified date and time.
    ...
    // Returns an array of all SleepMeasurement objects for the specified user, sorted by date
    public static function getMeasurementsBy($byAttrName, $byAttrValue, $order = 'desc') {
        ...
        try {
            ...
            if (!in_array($byAttrName, $allowedByAttrs))
                throw new PDOException("$byAttrName not allowed search criterion for sleep measurement");
            ...
            // collect returned data in array of SleepMeasurement objects
            foreach ($stmt as $row) {
    ...
    /* Returns an array of SleepMeasurement objects for the specified user, sorted by date.
     ...
     * If $groupEachDay is false, this returns an array of stdClass objects representing the average INDIVIDUAL sleep entry over each $timePeriod.
     * In other words, it would return the average duration of INDIVIDUAL sleep entries for each month, if $timePeriod was "month".
     * If $groupEachDay is true, the stdClass objects returned instead represent the average DAILY sleep over each $timePeriod.
     * In other words, it would return the average amount of sleep PER DAY for each month, if $timePeriod was "month".
