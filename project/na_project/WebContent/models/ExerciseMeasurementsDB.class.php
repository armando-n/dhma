...
class ExerciseMeasurementsDB {
    ...
    /* Adds the ExerciseMeasurement object that must be passed as the first argument.
    ...
    /* Returns an array of all ExerciseMeasurement objects found in the database for all users.
    ...
    public static function getAllMeasurements() {
        ...
        try {
            $db = Database::getDB();
            $stmt = $db->prepare(
                "select userName, dateAndTime, duration, type, units, notes
    ...
    // Returns an ExerciseMeasurement object for the specified user with the specified date and time.
    ...
    // Returns an array of all ExerciseMeasurement objects for the specified user, sorted by date
    public static function getMeasurementsBy($byAttrName, $byAttrValue, $order = 'desc') {
        ...
        try {
            ...
            if (!in_array($byAttrName, $allowedByAttrs))
                throw new PDOException("$byAttrName not allowed search criterion for exercise measurement");
            ...
            // collect returned data in an array of ExerciseMeasurement objects
            foreach ($stmt as $row) {
    ...
    /* Returns an array of ExerciseMeasurement objects for the specified user, sorted by date.
     ...
     * If $groupEachDay is false, this returns an array of stdClass objects representing the average INDIVIDUAL exercise entry over each $timePeriod.
     * In other words, it would return the average duration of INDIVIDUAL exercise entries for each month, if $timePeriod was "month".
     * If $groupEachDay is true, the stdClass objects returned instead represent the average DAILY exercise over each $timePeriod.
     * In other words, it would return the average amount of exercise PER DAY for each month, if $timePeriod was "month".
